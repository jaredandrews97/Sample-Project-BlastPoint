...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_fp", type=str, help="File path for the raw training data", required=False,
                        default=raw_data_fp)
    parser.add_argument("--wandb_log", type=bool, help="Boolean indicating if results logged as weights and bias run",
                        required=False, default=False)
    parser.add_argument("--preprocess_data", type=bool, help="Indicate if you only want to perform data preprocessing "
                                                             "or to also train a model", required=False, default=True)
    parser.add_argument("--train_model", type=bool, help="Indicate if you only want to perform data preprocessing "
                                                         "or to also train a model", required=False, default=False)
    parser.add_argument("--model", type=str, help="ML model algorithm to train",
                        choices=['LogisticRegression', 'RandomForest', 'XGBoost'], required=False, default=None)
